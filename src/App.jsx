...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: "340px", background: "#0a0d14", border: "1px solid #1e293b", borderRadius: "10px", padding: "36px 32px" }}>
        <div style={{ marginBottom: "28px" }}>
          <div style={{ color: "#60a5fa", fontSize: "8.5px", letterSpacing: "3px", marginBottom: "6px" }}>SISTEMA DE GESTIÓN DE PRODUCCIÓN</div>
          <div style={{ color: "#f1f5f9", fontSize: "18px", fontWeight: "700" }}>ACCESO</div>
          <div style={{ color: "#334155", fontSize: "9px", marginTop: "4px" }}>Mapa Interactivo Editable</div>
        </div>

        <form onSubmit={handleLogin}>
          <div style={{ marginBottom: "14px" }}>
            <div style={{ color: "#64748b", fontSize: "9px", letterSpacing: "1px", marginBottom: "5px" }}>EMAIL</div>
            <input
              type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus
              style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "5px", padding: "9px 12px", color: "#f1f5f9", fontSize: "12px", width: "100%", fontFamily: "inherit", boxSizing: "border-box", outline: "none" }}
            />
          </div>
          <div style={{ marginBottom: "20px" }}>
            <div style={{ color: "#64748b", fontSize: "9px", letterSpacing: "1px", marginBottom: "5px" }}>CONTRASEÑA</div>
            <input
              type="password" value={password} onChange={e => setPassword(e.target.value)} required
              style={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "5px", padding: "9px 12px", color: "#f1f5f9", fontSize: "12px", width: "100%", fontFamily: "inherit", boxSizing: "border-box", outline: "none" }}
            />
          </div>
          {error && <div style={{ color: "#ef4444", fontSize: "9.5px", marginBottom: "14px", background: "#1e0a0a", border: "1px solid #7f1d1d", borderRadius: "4px", padding: "6px 10px" }}>{error}</div>}
          <button type="submit" disabled={loading} style={{ width: "100%", background: loading ? "#1d4ed8" : "#3b82f6", border: "none", borderRadius: "5px", padding: "10px", color: "#fff", fontSize: "11px", fontWeight: "700", cursor: loading ? "wait" : "pointer", fontFamily: "inherit", letterSpacing: "1.5px" }}>
            {loading ? "VERIFICANDO…" : "ENTRAR"}
          </button>
        </form>

        <div style={{ marginTop: "24px", borderTop: "1px solid #1e293b", paddingTop: "14px", color: "#1e293b", fontSize: "8px", textAlign: "center" }}>
          Decoraciones Egea · Acceso restringido
        </div>
      </div>
    </div>
  );
...
/** Devuelve true si existe el edge inverso (B→A) para el edge dado (A→B) */
function hasPair(edge, edges) {
  return edges.some(e => e.id !== edge.id && e.from === edge.to && e.to === edge.from);
}

/**
 * Calcula el punto de control cuadrático con offset lateral perpendicular.
 * offset > 0 curva a un lado, < 0 al otro, 0 = curva vertical suave (comportamiento actual).
 */
function calcCurve(fc, tc, offsetMag) {
  const mx = (fc.x + tc.x) / 2;
  const my = (fc.y + tc.y) / 2;
  if (offsetMag === 0) {
    return { qx: mx, qy: my - 22 };
  }
  ...
  const px = -dy / len;
  const py = dx / len;
  return { qx: mx + px * offsetMag, qy: my + py * offsetMag };
}
...
export default function App() {
  ...
  if (session === undefined) {
    return <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: "#0f1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ color: "#334155", fontSize: "10px", letterSpacing: "2px" }}>CARGANDO…</span></div>;
  }
  if (!session) {
    return <LoginScreen />;
  }
          ...
          <svg ref={svgRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }} onMouseDown={onSvgDown}>
            ...
            <g transform={`translate(${pan.x},${pan.y}) scale(${zoom})`}>
              ...
              {edges.map(edge => {
                ...
                // Determinar offset lateral para bidireccionales
                const BIDIR_OFFSET = 45;
                const paired = hasPair(edge, edges);
                // El edge cuya id sea "menor" curva en positivo, el otro en negativo
                const inverso = paired ? edges.find(e => e.id !== edge.id && e.from === edge.to && e.to === edge.from) : null;
                const offsetMag = paired
                  ? (inverso && edge.id < inverso.id ? BIDIR_OFFSET : -BIDIR_OFFSET)
                  : 0;
              ...
              return (
                <div key={pid} style={{ marginBottom: "12px", borderLeft: `2px solid ${ps.color}`, paddingLeft: "9px" }}>
                  ...
                  <div style={{ marginTop: "7px", display: "flex", flexWrap: "wrap", gap: "3px" }}>
                    {prob.nodes.map(nid => {
                      const n = nodes.find(x => x.id === nid); const col = GC[n?.group] || GC.sistema; return (
